...
type HttpServerOption func(s *sdkHttpServer)

// 不使用template的可以不初始化templateEngine
...
func NewServer(name string, options ...HttpServerOption) Server {
	...
	sdkHttpServer := &sdkHttpServer{Name: name, handler: handler, root: root, ctxPool: sync.Pool{
		New: func() interface{} {
			return NewContext()
		},
	}}
